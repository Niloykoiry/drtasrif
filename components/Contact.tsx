...
export default function Contact() {
  ...
  const items = [
    ...
    { icon: <Phone className="h-4 w-4" />, value: t.appt.phoneValue },
    { icon: <Mail className="h-4 w-4" />, value: t.appt.emailValue },
  ];
  ...
  return (
    <section id="contact" className="bg-white py-24">
      ...
      >
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-3">
            {items.map((item) => (
              <div key={item.value} className="flex min-w-0 items-start gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-sky-100 text-blue-700">
                  {item.icon}
                </span>
                <div className="min-w-0">
                  <p className="break-words text-[13px] font-bold text-navy">
                    {item.value}
                  </p>
        ...
        <motion.div
          variants={fadeUp}
          className="relative h-72 overflow-hidden rounded-3xl border border-slate-100 shadow-inner sm:h-80"
        >
          <MapBackdrop />
          <div className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center">
            ...
            <span className="mt-1 rounded-full bg-white/90 px-2.5 py-0.5 text-[10px] font-bold text-slate-600 shadow">
              {t.contact.mapCity}
            </span>
          </div>
          <div className="absolute right-4 top-1/2 w-44 -translate-y-1/2 rounded-2xl bg-white/90 p-4 shadow-glass backdrop-blur">
            ...
            <a
              href="https://www.google.com/maps/place/196+Green+Rd,+Dhaka+1205/@23.7476424,90.383183,1032m/data=!3m2!1e3!4b1!4m6!3m5!1s0x3755b8ba5915c691:0xe9c3bb39c327047f!8m2!3d23.7476375!4d90.3857579!16s%2Fg%2F11sd4621w5?entry=ttu&g_ep=EgoyMDI2MDkxNC4wIKXMDSoASAFQAw%3D%3D"
              target="_blank"
