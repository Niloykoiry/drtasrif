...
export default function About() {
  ...
  return (
    <section
      id="about"
      className="relative bg-gradient-to-b from-white via-sky-50/60 to-white py-24"
    >
      <motion.div
        variants={stagger}
        initial="hidden"
        ...
        className="container-x grid items-center gap-12 lg:grid-cols-[1fr_1.05fr_1fr]"
      >
        ...
        <motion.div
          variants={fadeUp}
          className="glass divide-y divide-slate-100/80 rounded-3xl bg-white/70 shadow-soft"
        >
          {t.about.cards.map((card, i) => (
            <div key={card.title} className="flex items-start gap-4 p-5">
              <span
                className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${GRADIENTS[i]} text-white shadow-md`}
              >
                {ICONS[i]}
              </span>
              <div>
        ...
        >
          <div
            aria-hidden="true"
            className="absolute -left-8 -top-8 h-40 w-40 rounded-[45%_55%_50%_50%/55%_45%_55%_45%] bg-gradient-to-br from-violet-200/70 to-teal-200/70 blur-sm"
          />
          <div className="relative overflow-hidden rounded-[2.2rem] shadow-glass">
            ...
            <div
              aria-hidden="true"
              className="absolute inset-0 bg-gradient-to-t from-blue-900/35 via-transparent to-transparent"
            />
      ...
      <Modal
        ...
        title={t.about.modal.title}
      >
        <h3 className="pr-10 text-xl font-extrabold text-navy">
          {t.about.modal.title}
        </h3>
